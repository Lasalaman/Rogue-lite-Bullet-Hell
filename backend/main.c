 ...
 *
 * 注意：必須先比對 "restart" 再比對 "start"。
 * "restart" 字串內含 "start" 子字串，若順序顛倒會把 restart 誤判為 start，
 * 導致 PLAYING 狀態下無法重開。
 */
static void parse_input_line(Game *g, const char *line) {
    if (strstr(line, "\"action\":\"restart\"")) {
...
/**
 * game_init - 初始化 Game 結構（stack 上單一實例，由 main 傳入指標）
 *
 * g 為指向 Game 的指標；透過 g->enemy_head / g->loot_head 管理堆積記憶體，
 * bullets 為 struct 內嵌固定陣列，不需 malloc/free。
 */
static void game_init(Game *g) {
    memset(g, 0, sizeof(*g));
...
static void spawn_normal_enemy(Game *g) {
  ...
  /* enemy_list_push 內部 malloc；失敗時回傳 NULL，不可計入已生成數 */
  node = enemy_list_push(g, x, y, hp, 0);
  if (node == NULL) {
    fprintf(stderr, "[engine] 警告：敵人 malloc 失敗，略過本次生成\n");
    return;
  }
...
static void spawn_boss(Game *g) {
  ...
  if (boss == NULL) {
    fprintf(stderr, "[engine] 警告：Boss malloc 失敗，下 tick 重試\n");
    return;
  }
  boss->boss_pattern = 0;
...
/** 敵人 AI：沿 enemy_head 鏈結串列走訪，e = e->next 逐節點前進 */
static void update_enemy_ai(Game *g, float dt) {
  Player *p = &g->player;
...
/**
 * resolve_collisions - 碰撞：子彈↔敵人、Boss 彈↔玩家、玩家↔ Loot
 *
 * 指標安全要點（鏈結串列遍歷 + 刪除）：
 *   - 遍歷敵人時若需刪除當前節點，必須先儲存 e->next，再呼叫 enemy_list_remove_node。
 *     remove 會 free(cur)，若先 free 再讀 cur->next 即為 use-after-free（Segfault）。
 *   - Loot 迴圈使用 lnext = l->next 預存下一節點，同理避免刪除後懸空指標。
 *   - 子彈使用固定陣列 g->bullets[i]（&g->bullets[i] 為合法位址），
 *     i 嚴格限制在 [0, MAX_BULLETS)，不會越界。
 */
static void resolve_collisions(Game *g) {
  int i;
  Player *p = &g->player;
      ...
      while (e != NULL) {
        ...
        if (dist2 <= (er + BULLET_RADIUS) * (er + BULLET_RADIUS)) {
          ...
          if (e->hp <= 0.0f) {
            ...
            if (loot_list_push(g, e->x, e->y, lt) == NULL) {
              fprintf(stderr, "[engine] 警告：掉落物 malloc 失敗，略過本次掉落\n");
            }
            g->score += e->is_boss ? 2000 : 10;
            {
              /*
               * 先快取 next 指標再 free：dead 節點被釋放後，
               * dead->next 不可再讀取，否則 Segmentation Fault。
               */
              Enemy *dead = e;
              e = e->next;
...
int main(void) {
  ...
  while (1) {
    /* 每 tick 排空 stdin，只保留最後一筆移動座標，避免前端高頻 POST 堆積造成延遲 */
    while (poll_stdin_line(line, sizeof(line))) {
