...
/**
 * bullet_pool_spawn - 在固定陣列池中找第一個 active==0 的槽位
 *
 * 設計取捨：子彈高頻生成/銷毀，若每次 malloc/free 會造成堆積碎片；
 * 改用 Game.bullets[MAX_BULLETS] 預配置，僅翻轉 active 旗標即可重用，
 * 指標 &game->bullets[i] 永遠指向合法 struct 記憶體（無 dangling risk）。
 */
